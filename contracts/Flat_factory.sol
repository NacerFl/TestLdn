...
// File contracts/MeerkatFactory.sol

pragma solidity =0.5.16;


contract MeerkatFactory is IMeerkatFactory {
    address public feeTo;
    address public feeToSetter;
    bytes32 public INIT_CODE_HASH = keccak256(abi.encodePacked(type(MeerkatPair).creationCode));
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, 'Meerkat: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'Meerkat: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'Meerkat: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(MeerkatPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        IMeerkatPair(pair).initialize(token0, token1);
    ...
    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, 'Meerkat: FORBIDDEN');
        feeTo = _feeTo;
    }
    ...
    function setSwapFee(address _pair, uint32 _swapFee) external {
        require(msg.sender == feeToSetter, 'Meerkat: FORBIDDEN');
        MeerkatPair(_pair).setSwapFee(_swapFee);
    }
}